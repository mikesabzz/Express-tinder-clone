...
class UserList extends React.Component {
  ...
  renderCreateBioButton = () => {
      return (
        <div className="button">
          <Link className="create-bio-button" to="/dashboard/create">
            Create Your Profile
          </Link>
        </div>
      );
  ...
  renderUsers = () => {
    ...
    if (genderInterest) {
      return genderInterest.map((interest) => {
        if (interest.bio && interest.bio.gender_preference) {
        ...
        return null; // Add this line to handle the case when bio or gender_preference is null
      });
    } else {
      return <div></div>;
    }
  ...
  render() {
    const currentUserHasBio = this.state.currentUser.some((user) => user.bio);
    return (
      <div className="dashboard">
        <h1>Welcome to Tinder Friendly</h1>
        <h3>Find out whos near you</h3>
        {!currentUserHasBio ? (
          <div>{this.renderCreateBioButton()}</div>
        ) : (
          <div>
